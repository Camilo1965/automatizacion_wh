...
- [ ] Add assertions that owner has `shipping:manage`, operator lacks it, and operator still has `shipping:operate`.
- [ ] Add HTTP assertions that an operator can read preferences and request shipping operations but gets 403 on carrier-rule, global-policy, and municipal-policy mutations.
- [ ] Run `pnpm --filter @camila/api exec vitest run --project unit test/capabilities.test.ts test/admin-shipping-http.test.ts` and confirm the new authorization assertion fails before implementation.
- [ ] Add the capability, pass a separate owner-only authenticator, and use it for `PUT /shipping/carrier-rules`, `PATCH /shipping/preferences`, `POST /shipping/rules`, `PATCH /shipping/rules/:localityCode`, and `POST /shipping/rules/:localityCode/deactivate`.
- [ ] Re-run the targeted tests and confirm owner writes and operator operations pass while operator policy writes return 403.
...
- [ ] Add a test where `attachPdf` persists the storage key and then throws; confirm it currently rejects and deletes the referenced object.
- [ ] Run `pnpm --filter @camila/api exec vitest run --project unit test/shipping-guide-service.test.ts` and confirm the new scenario fails for the expected reason.
...
- [ ] Re-run the targeted test and the complete `shipping-guide-service.test.ts` suite, including the existing concurrent-winner case.
...
- [ ] Start only the disposable `postgres-test` service before integration tests; never redirect tests to `.env`'s local business database.
- [ ] Run `pnpm verify` and `pnpm security:secrets`.
